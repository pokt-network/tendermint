...
func UnsafeRollbackData(config *cfg.Config, modifyStateFile bool, height int64) error {
	blockStore, state, blockStoreDB, stateDb, err := BlocksAndStateFromDB(config, DefaultDBProvider)
	if err != nil {
		return err
	}
	...
	if state.LastBlockHeight > height {
		...
		stateRestore := restoreStateFromBlock(stateDb, blockStore, height)
		// save the state to the db to overwrite the current state
		SaveState(stateDb, stateRestore)
		...
		// set the blockstore pointer to the new blockstore
...
func DefaultDBProvider(ctx *DBContext) (db.DB, error) {
	dbType := db.DBBackendType(ctx.Config.DBBackend)
...
func RollbackTxIndexer(config *cfg.Config, height int64) error {
	...
	return txIndexer.DeleteFromHeight(height)
}
